...
function renderCurrentView() {
  ...
  renderCards(); renderTable(); queueVisiblePhotos();
}
...
function renderTable() {
  $('taxaTableBody').innerHTML = pageRows().map(r => {
    const interaction = safe(r.Interaction_Summary) || 'No host/interaction record in merged layers';
...
function showDetailById(id, scroll = true) {
  ...
  if (!r) return;
  selectedId = id;
  const interaction = safe(r.Interaction_Summary) || 'No host/interaction example in the merged layers.';
